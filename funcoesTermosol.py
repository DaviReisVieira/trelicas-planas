...
def insert_constraints(list, node, direction):
    if len(list) == 0 or list[-1][0] > node:
        list.append([node, direction])
        return list

    for i, constraint in enumerate(list):
        if (constraint[0] == node and constraint[1] < direction) or (constraint[0] < node):
            return [*list[:i], [node, direction], *list[i:]]
    return list